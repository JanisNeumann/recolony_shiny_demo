...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Relative Abundance of Select Clostridiales"),
  sidebarLayout(
    sidebarPanel(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Box Plots", plotOutput("box_plots", height = 1000, width = 600)),
        tabPanel("Density Plots", plotOutput("density_plots", height = 1000, width = 600)),
        tabPanel("UMAP", plotOutput("umap_plot", height = 750, width = 750))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  color_vector <- c(alpha("grey59", alpha = 0.5),
                    viridis(2, alpha = 0.5, begin = 0.4, end = 0.7, option = "D"))
  ...
  # Reformat data for UMAP
  df_data_umap <- reactive({
    df_data_umap <- df_data()
    if (is.null(df_data_umap)) {
      return(NULL)
    }
    
    df_data_umap <- df_data_umap %>%
  ...
  output$box_plots <- renderPlot({
    ...
    if(is.null(box_plots_df)){return()}
  ...
  output$density_plots <- renderPlot({
    ...
    if(is.null(density_plots_df)){return()}
